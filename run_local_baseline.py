...
# =====================================================================
# Purpose: Baseline 1H backtest (reference)
# Data:    data/BTC_USD_1h_frozen.csv (frozen snapshot)
# =====================================================================
...
def load_data():
    print("Loading BTC-USD 1H from frozen CSV...")
    df = pd.read_csv("data/BTC_USD_1h_frozen.csv",
    ...
    if len(df) < 1000:
        raise SystemExit("Not enough data in frozen CSV.")

    print(f"Loaded {len(df)} bars from frozen CSV.")
    return df
            ...
            if i >= PATTERN_LEN - 1:
                ...
                if count >= K_NEIGHBORS:
                    ...
                    if K_NEIGHBORS < len(distances):
                        top = np.argpartition(
                            distances, K_NEIGHBORS - 1)[:K_NEIGHBORS]
                    else:
                    ...
                    if weight_sum > 0:
                        raw_prediction = float(
                            (weights * y_top).sum() / weight_sum)

            if raw_prediction is not None:
                if pred_ema is None:
                    pred_ema = raw_prediction
                else:
                    pred_ema = (ema_alpha * raw_prediction +
                                (1.0 - ema_alpha) * pred_ema)
