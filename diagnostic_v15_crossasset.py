...
# =====================================================================
# Purpose: Cross-asset robustness check
# Assets:  BTC-USD, ETH-USD, SOL-USD
# Method:  6 chronological windows per asset
# Config:  4h timeframe, MTF on, kNN on, trailing disabled
# Data:    data/{ASSET}_USD_4h_frozen.csv (frozen snapshots)
# =====================================================================
...
def load_asset(symbol):
    print(f"  Loading {symbol} 4H from frozen CSV...")
    path = f"data/{symbol.replace('-', '_')}_4h_frozen.csv"
...
def run_kfold_for_asset(symbol, df):
    ...
    std_net = np.std([r["net"] for r in results],
                     ddof=1) if N_FOLDS > 1 else 0.0
...
def print_asset_folds(asset_result):
    ...
    t_str = f"{asset_result['t_stat']:+.3f}" if np.isfinite(
        asset_result['t_stat']) else "n/a"
...
def main():
    print("=" * 105)
    print("CROSS-ASSET VALIDATION (6 windows per asset)")
    print("=" * 105)
    print(f"Assets:      {', '.join(ASSETS)}")
    print(f"Timeframe:   {TIMEFRAME}")
    print(f"Windows:     {N_FOLDS}")
